    ...
    public function columns(): array
    {
        return [
            TD::make('title', __('Title'))
                ...
                ->cantHide()
                ->filter(Input::make())
                ->render(static fn (Site $site) => Link::make($site->title)
            ...
            TD::make('home_link', __('Home link'))
                ->cantHide()
                ->filter(Input::make())
                ->render(static fn (Site $site) => Link::make(__('Open'))
                    ->href($site->home_link)->target('_blank')),

            TD::make('fed_at', __('Fed'))
                ->sort()
                ->render(static fn (Site $site) => $site->fed_at?->toDateTimeString()),

            TD::make('created_at', __('Created'))
                ->sort()
                ->render(static fn (Site $site) => $site->created_at->toDateTimeString()),
        ];
    }
}
